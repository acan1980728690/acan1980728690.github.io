...
> 此篇来源：https://github.com/Seazean/JavaNote/blob/main/Prog.md,建议结合着老师的笔记文件和代码看：https://gitee.com/linzm1007/heima-manyihang-juc，如果一定要单独看一篇的话看这篇，老师的笔记多了很多例子，不过这些例子自己去翻代码能找到大部分。但是笔记里不知道为什么少了很多知识点没写
...
| public void interrupt()                     | 打断这个线程，将打断标记设置为true,当线程处于可中断的阻塞方法时（如join，sleep方法），抛出InterruptedException异常，并且会重新把打断标记设置为false。 |
...
public class Test {
    ...
    private static void test1() throws InterruptedException {
        ...
        t1.start();
        //等待线程执行结束，输出10,如果没有t1.join就不会等待，那么输出就会是0,并且可以设置最大等待时间（单位为毫秒），如果是t1.join(2000),一秒后就已经等到了，直接结束并且输出10,不会等2秒，如果是t1.join(500),则会等0.5秒，然后不等了输出0
        t1.join();
...
> park简单来说就是会让线程进入阻塞状态，正常需要通过unpark释放这个阻塞状态，就像是一个无限时间的sleep，要手动用unpark唤醒或者interrupt打断
...
一个程序运行多个线程本身是没有问题的，多个线程读共享资源其实也没有问题，问题出在在多个线程对共享资源读写操作时发生指令交错
...
* 锁对象建议使用共享资源
* 在实例方法上使用 synchronized 时会使用 this 作为锁对象
* 在静态方法上使用 synchronized 时会使用 类名.class 字节码作为锁对象，因为静态成员属于类，被所有实例对象共享，所以需要锁住类
...
public static void main(java.lang.String[]);
 ...
 Code:
 stack=2, locals=3, args_size=1 //前面的不重要
 0: getstatic #2 // <- 获取lock引用 （synchronized开始）
 3: dup //复制一份lock引用（monitorenter会消耗一份引用）
 4: astore_1 // lock引用 -> slot 1(存到1中)
 5: monitorenter // 将 lock对象 MarkWord 置为 Monitor 指针（如果锁已经被人获取了，那么线程就会阻塞在这里并进入monitor对象的entryList）
 6: getstatic #3 // <- i (从3中获取i)
 9: iconst_1 // 准备常数 1 
 10: iadd // +1 
 11: putstatic #3 // -> i (把i放回3里)
 14: aload_1 // <- lock引用 (从1中获取lock引用)
 15: monitorexit // 将 lock对象 MarkWord 重置, 唤醒 EntryList
 16: goto 24 //直接跳转到24,即直接退出，下面的是异常处理
 19: astore_2 // e -> slot 2 （异常放到2中）
 20: aload_1 // <- lock引用 （从1中获取lock引用）
 21: monitorexit // 将 lock对象 MarkWord 重置, 唤醒 EntryList
 22: aload_2 // <- slot 2 (e) （从2中获取异常）
 23: athrow // throw e （抛出异常）
 24: return
 //异常表，意思是6-16有异常了会跳转到19,19-22有异常了也会跳转到19
 Exception table:
 from to target type
 6 16 19 any
 19 22 19 any
...
CAS的意思是Compare And Swap，即比较并交换，是一种无锁算法，是一种原子操作，也是乐观锁的一种（乐观锁即认为不会或者很少发生线程冲突问题，先操作，操作完了如果发现已经被改了就重试或者直接失败）。用伪代码表示大概是这样的
...
其中 V 代表要更新的变量，A 代表预期值，B 代表新值。如果 V 的值等于 A，则将 V 的值更新为 B，并返回 true，否则返回 false。

自旋的意思是失败后自己会重试，例如下面的例子
...
public final int incrementAndGet() {
    for (;;) {
        ...
        if (compareAndSet(current, next)) // CAS 尝试更新
            return next;
        // 如果失败（说明被其他线程改了），继续循环重试
    }
}
```

MarkWord是对象头中的一部分,有多种状态，以32位的为例子

![](Prog/20251226134902.png)

##### 偏向锁

偏向锁的思想是偏向于让第一个获取锁对象的线程，这个线程之后重新获取该锁不再需要同步操作：

* 当锁对象第一次被线程获得的时候进入偏向状态，标记为 101（Mark Word最后面的3位），同时**使用 CAS 操作将线程 ID 记录到 Mark Word**。如果 CAS 操作成功，当线程再次执行到这个synchronized的时候，JVM通过锁对象的Mark Word判断 ：当前线程ID还存在，还持有这个对象的锁，于是就可以继续进入临界区执行，而不需要再次获得锁
...
* 首先，JVM会将锁对象的Mark Word恢复成为无锁状态，在当前线程的栈桢中分配一个空间，叫做Lock Record，把锁对象obj的Mark Word复制了一份，官方称为：Displaced Mark Word

> 对于下面的图的说明，obj和Lock Record的第一个头都是Mark Word,Lock Record的图应该错了，Lock Record没有MarkWord，只有一个Displaced Mark Word存储obj的Mark Word信息

  ![](https://seazean.oss-cn-beijing.aliyuncs.com/img/Java/JUC-轻量级锁原理1.png)

* 让锁记录中 Object reference 指向锁住的对象，并尝试用 CAS 替换 Object 的 Mark Word
...
* 如果 CAS 失败，有两种情况：
  ...
  * 这个锁对象已经被其他线程抢占了，那么它就会自旋等待锁，一定次数后仍未获得锁对象，说明发生了竞争，需要膨胀为重量级锁。
  * 如果是线程自己执行了 synchronized 锁重入，就添加一条 Lock Record 作为重入的计数，新的Lock Record的Displaced Mark Word为null
...
* 当退出 synchronized 代码块（解锁时）
  ...
  * 如果锁记录的值不为 null，这时使用 CAS **将 Displaced Mark Word 的值恢复给obj的对象头**
    * 成功，则解锁成功
...
假设是非重入锁，那么就只有execute method1了，因为之后都获取不到锁了
...
Thread t1 = new Thread(() -> {
  log.debug("启动...");
  try {
    lock.lockInterruptibly();//尝试获取锁，如果被打断，则抛出 InterruptedException
  } catch (InterruptedException e) {
    e.printStackTrace();
    log.debug("等锁的过程中被打断");
    return;
...
//线程先用lock.lock()获得锁，然后启动线程，t1线程等锁，等的时候被打断了立即抛出异常
```
...
> 以下这段代码不一定会插入成功，但改为公平锁后（设置为true）一定是在最后的
```java
...
public final native void wait(long timeout):有时限的等待, 到n毫秒后或者被唤醒后结束等待
```
